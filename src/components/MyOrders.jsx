...
const MyOrders = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const resultOrders = await axios.get(`https://jigit-api.onrender.com/api/orders/mine/${userId}`);
              ...
              <div className="mo-groups">
                {
                  orders?.map((item) => (
                    <div key={item._id} className="mo-group">
                      ...
                      <Link to={`/order/${item._id}`}><HiEye className='text-red-600 hover:text-gray-300' size={20} /></Link>
                    </div>))
                }
              </div>
            )
      ...
      <div className="flex flex-col gap-10">
        {
          orders.length === 0 ? (
            ...
            <div>
              {
                orders?.map((item) => (
                  <div key={item._id} className="flex justify-between border-b-[1px] border-black">
